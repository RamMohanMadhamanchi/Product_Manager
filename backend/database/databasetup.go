...
func DBSet() *mongo.Client {
	client, err := mongo.NewClient(options.Client().ApplyURI("mongodb+srv://admin:admin@cluster0.zmpk3.mongodb.net/ProductManagerdata?retryWrites=true&w=majority"))
	if err != nil {
		log.Fatal(err)
	}
...
func UserData(client *mongo.Client, CollectionName string) *mongo.Collection {
	var collection *mongo.Collection = client.Database("ProductManager").Collection(CollectionName)
	return collection

}

func ProductData(client *mongo.Client, CollectionName string) *mongo.Collection {
	var productcollection *mongo.Collection = client.Database("ProductManager").Collection(CollectionName)
